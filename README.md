...
Due to the fact that the DINO model is no longer supported by the developers and the existing problems are not being solved, this fork of the official repository of the DINO model was created. Here we added the ability to easily logging with [wandb.ai](https://wandb.ai/site) and fixed an annoying error when trying to create a folder to start a workout. You can submit your changes in a pull request.
